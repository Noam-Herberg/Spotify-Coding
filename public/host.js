...
let state = null, spotifyPlayer = null, deviceId = null, playerPromise = null, tokenCache = null;
...
async function play(side) { const track = side === 'a' ? state.currentMatchup.songA : state.currentMatchup.songB; try { await initialisePlayer(); await spotifyPlayer.activateElement(); const response = await fetch(`https://api.spotify.com/v1/me/player/play?device_id=${encodeURIComponent(deviceId)}`, { method: 'PUT', headers: { Authorization: `Bearer ${await playbackToken()}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ uris: [track.uri] }) }); if (!response.ok) throw new Error(response.status === 403 ? 'Spotify Premium is required.' : 'Spotify could not play this track.'); await mutate('played', { side }); } catch (error) { text('#match-message', `${error.message} Use Open in Spotify or replace the track.`); } }
